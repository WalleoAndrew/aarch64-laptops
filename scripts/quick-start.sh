...
function usage()
{
    print_blue "USAGE: $0 [[asus-tp370ql|hp-envy-x2]lenovo-miix-630|lenovo-yoga-c630|huawei-planck|generic]|kernel|grub|help][-f]"
    return 1
}
...
while [ $# -gt 0 ]; do
    case $1 in
        asus-tp370ql|hp-envy-x2|lenovo-miix-630|lenovo-yoga-c630|huawei-planck|generic)
            DEVICE=$1
            ;;
        kernel)
            BUILD_KERNEL=true
